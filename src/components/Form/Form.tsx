...
type Props<Model extends BaseModel, ValidationModel extends Model = Model> = {
  ...
  ) => AsyncResult<void, SubmissionErrors<ValidationModel>>;
  onValidationErrorsUpdated?: (
...
> extends React.Component<Props<ValidationModel>, State<ValidationModel>> {
  ...
  private validateField(field: keyof ValidationModel) {
    ...
    this.setState((prev) => {
      const { [field]: removed, ...restPrevValidationErrors } =
        prev.errors.validationErrors || {};
  ...
  private calculateHasErrors({ errors }: State<Model, ValidationModel>) {
    const {
      validationErrors,
    ...
    return (
      !!(
        validationErrors &&
        !!objectKeys(validationErrors).find(
  ...
  private calculateHasChanges(p: State<Model, ValidationModel>) {
    return !!objectKeys(p.model).find(
      (key) =>
        !(
          this.props.initialModel &&
          p.model[key] === this.props.initialModel[key]
        )
    );
  }

  private calculateCanSubmit(p: State<Model, ValidationModel>) {
  ...
  private onChange(field: keyof ValidationModel, newValue: string) {
    ...
    this.setState((prev) => {
      ...
      } as unknown as State<Model, ValidationModel>;

      return {
        ...nextState,
        hasErrors: this.props.disableValidators
          ? true
          : this.calculateHasErrors(nextState),
        canSubmit: this.props.disableValidators
          ? true
          : this.calculateCanSubmit(nextState),
      } as State<Model, ValidationModel>;
    });
  ...
  componentDidUpdate(
    prevProps: Props<ValidationModel>,
    prevState: State<ValidationModel>
  ) {
